package com.ktfootball.app.calendar.widget;
...
public class CalendarView extends View {
    ...
    private int afterDay = 3;//能点击几天后的
    ...
    private void init(Context context) {
        ...
        mPayTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPayTextPaint.setTextSize(PhoneUtils.sp2px(getContext(), paySP));
        mPayTextPaint.setColor(Color.BLACK);
        initDate();
    ...
    private void measureClickCell(int col, int row) {
        ...
        if (WantToMakeActivity.currentCity != null) {
    ...
    private boolean whetherToClick(CustomDate date) {
        int day = afterDay * 1000 * 60 * 60 * 24;
        if (DateUtils.getTime(date.year + "-" + DateUtils.formatNum(date.month) + "-" + DateUtils.formatNum(date.day), "yyyy-MM-dd") > (DateUtils.getTodayMs() + day)) {
            return true;
        } else {
            return false;
        }
...
//            String dateStr = date.year+"-"+date.month+"-"+date.day;
            LogUtils.e(dateStr);
            float contentX = (float) ((i + 0.5) * mCellSpace - mTextPaint.measureText(content) / 2);
            float contentY = (float) ((j + 0.7) * mCellSpace - mTextPaint.measureText(content, 0, 1) / 2);
            canvas.drawText(content, contentX, contentY, mTextPaint);
            String pay = WantToMakeActivity.selectPay(dateStr);
//            String pay = "￥999";
            canvas.drawText(pay,
                    (float) ((i + 0.5) * mCellSpace - mPayTextPaint.measureText(pay) / 2),
                    (float) ((j + 0.7) * mCellSpace - mPayTextPaint.measureText(
                            content, 0, 1) / 2) + PhoneUtils.dip2px(getContext(), paySP + 2), mPayTextPaint);
        }
    }
    ...
    private void fillMonthDate() {
        ...
        LogUtils.e("lastMonthDays = " + lastMonthDays + "...currentMonthDays = " + currentMonthDays + "...firstDayWeek = " + firstDayWeek);
