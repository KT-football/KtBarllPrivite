...
import com.ktfootball.app.Request.StudyFinishedRequest;
import com.ktfootball.app.Views.PauseDialog;
import com.ktfootball.app.Views.TrainCloseDialog;
...
public class VedioStartActiviy extends BaseActivity {
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        videos = (AppCartoon.Videos) getIntent().getSerializableExtra("video");
        appCartoon = (AppCartoon) getIntent().getSerializableExtra("data");

        progressBar.setMax(Integer.parseInt(videos.total_times));
        totolNum.setText("/" + videos.total_times);
        time = (long) (Float.parseFloat(videos.speed) * 1000/2);
        totol = Integer.parseInt(videos.total_times);
...
//        BitmapManager.getInstance().display(bg, bgPath);
        ...
        for (int x = 0; x < fileSize; x++) {
            ...
            frameAnim.addFrame(getDownDrawable(imagePath), (int) (Float.parseFloat(videos.speed) * 1000 / fileSize));
        }
            ...
            timerTask = new TimerTask() {
                ...
                public void run() {
                    ...
                    if (num == totol) {
                        doStudyFinished();
                        return;
                    }else if(num>Integer.parseInt(videos.total_times)){
    ...
    private void doStudyFinished() {
        ...
        request.add("app_cartoon_video_id", videos.id);
        request.add("study_second", videos.total_times);
...
//        timer.purge();
    ...
    private void downLessons(String str, AppCartoon.Lessons lessons, int code) {
        ...
        intent.putExtra("path", path + lessons.name);
        intent.putExtra("data", appCartoon);
