...
public class ColorArcProgressBar extends View {
    ...
    private String bgArcColor = "#babcbd";
...
//        isNeedTitle = a.getBoolean(R.styleable.ColorArcProgressBar_is_need_title, false);
    ...
    public void setHintPaintColor(int color) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            canvas.drawText(String.format("%.0f", curValues) + "%", centerX, centerY + 15 + hintSize, vTextPaint);
        }
        if (isNeedUnit) {
            canvas.drawText(hintString, centerX, centerY + textSize / 3 - 15, hintPaint);
        }
        if (isNeedTitle) {
